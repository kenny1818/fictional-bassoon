...
// ADD
LOCAL _HMG_ControlHandle, _HMG_MouseRow_, _HMG_MouseCol_, _HMG_ControlContextMenu
...
*/
   ...
   For i := 1 To HMG_LEN ( _HMG_SYSDATA [ 60 ] )
      ...
      if valtype( cProc ) == 'B' //20111107 kenny ooi
        // { |hWnd ,nMsg ,wParam ,lParam|userfunc( hWnd ,nMsg ,wParam ,lParam ) } ) 
        r := eval( cProc ,hWnd , nMsg , wParam , lParam )
      elseif valtype( cProc ) == 'O' //20111107 kenny ooi
        r := cProc:HandleEvent( hWnd , nMsg , wParam , lParam )
      elseif valtype( cProc ) == 'C'
...
/*
		...
		EndIf
*/
 if _HGX_F1help //20111108 kenny ooi disable f1Help
		i := ascan ( _HMG_SYSDATA [3] , GetHelpData ( lParam ) )
...
// Dr. Claudio Soto (May 2013)
   ...
   _HMG_MouseRow_  := HIWORD(lParam)
   _HMG_MouseCol_  := LOWORD(lParam)
   ...
   IF _HMG_SetControlContextMenu == .T. 
        ...
        IF i > 0
            ...
            TrackPopupMenu ( _HMG_ControlContextMenu , _HMG_MouseCol_ , _HMG_MouseRow_ , hWnd )
        ENDIF
   ENDIF
...
Local ActiveWindowHandle
	...
	EndIf
if _HMG_SYSDATA [ 69 ] [ i ] == 'P'  //kenny ooi
  _HMG_SYSDATA [ 81 ] [ i ] := .f.
endif
   ...
   nLastWindowIndex := GetFormIndex( aForm [nForm] )
	...
	If nForm > 1
		...
		For z := 1 to nForm
          ...
          if !_HMG_SYSDATA [ 81 ] [ x ] // kenny ooi
...
*------------------------------------------------------------------------------*
   IF _HMG_StopControlEventProcedure [i] == .T. .or. _HGX_StopAllEvent // 20111112 kenny ooi ( Dr. Claudio Soto, April 2013 )
      RETURN .F.
   ENDIF
...
Local lRetVal := .F.
   ...
   IF _HMG_StopWindowEventProcedure [i] == .T. .or. _HGX_StopAllEvent  // 20111112 kenny ooi  ( Dr. Claudio Soto, April 2013 )
      RETURN .F.
   ENDIF
...
_HGX_StopAllEvent := IIF (ValType(lStop) <> "L", .F., lStop )
RETURN NIL
*------------------------------------------------------------------------------*
...
*------------------------------------------------------------------------------*
function InstallEventHandler ( cProcedure )
*------------------------------------------------------------------------------*
local lnret := 0 ,lnpos
lnpos := ascan( _HMG_SYSDATA [ 60 ] ,nil ) 
if valtype( cProcedure ) == 'C'
  if lnpos > 0
    _HMG_SYSDATA [ lnpos ] := ALLTRIM ( HMG_UPPER ( cProcedure ) )
  else
	  aadd ( _HMG_SYSDATA [ 60 ] , ALLTRIM ( HMG_UPPER ( cProcedure ) ) )
    lnpos := len( _HMG_SYSDATA [ 60 ] )
  endif
elseif valtype( cProcedure ) == 'B' //codeblock 20111107 kenny ooi
        // { |hWnd ,nMsg ,wParam ,lParam|userfunc( hWnd ,nMsg ,wParam ,lParam ) } ) 
  if lnpos > 0
    _HMG_SYSDATA [ lnpos ] := cProcedure
  else
	  aadd ( _HMG_SYSDATA [ 60 ] , cProcedure ) ; lnpos := len( _HMG_SYSDATA [ 60 ] ) 
  endif
elseif valtype( cProcedure ) == 'O' //codeblock 20111107 kenny ooi
        // { |hWnd ,nMsg ,wParam ,lParam|userfunc( hWnd ,nMsg ,wParam ,lParam ) } ) 
  if __objHasMethod( cProcedure ,'HandleEvent' )
    if lnpos > 0
      _HMG_SYSDATA [ lnpos ] := cProcedure
    else
	    aadd ( _HMG_SYSDATA [ 60 ] , cProcedure ) ; lnpos := len( _HMG_SYSDATA [ 60 ] ) 
    ...
    msginfo( 'Object mush have HandleEvent Method' )
  endif
elseif valtype( cProcedure ) == 'N' //remove user events 20111108 kenny ooi
  if len( _HMG_SYSDATA [ 60 ] ) >= cProcedure .and. cProcedure > 0
    _HMG_SYSDATA [ 60 ] [ cProcedure ] := nil
    lnpos := cProcedure
  else
    lnpos := -1
  endif
else
  msginfo( 'Invalid user function/procedure type "InstallEventHandler"' )
endif
