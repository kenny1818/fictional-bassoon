...
#ifdef UNICODE  //20210926 kenny ooi
   LPWSTR lpsIniDir, lpsTitle;
   LPSTR  pRetStr;
#endif
...
#ifdef UNICODE  //20210926 kenny ooi
   lpsIniDir = HMG_parc(3); lpsTitle = HMG_parc(2);
   ofn.lpstrInitialDir  = lpsIniDir ; //(LPCTSTR) HMG_parc(3); 20210926
   ofn.lpstrTitle       = lpsTitle ; //(LPCTSTR) HMG_parc(2);
#else
...
#ifdef UNICODE  //20210926 kenny ooi
{       pRetStr = HMG_WCHAR_TO_CHAR( ofn.lpstrFile );
        hb_retc( pRetStr );
        hb_xfree( pRetStr ); }
#else
...
#ifdef UNICODE  //20210926 kenny ooi
              {pRetStr = HMG_WCHAR_TO_CHAR( cFullName[ i ] );
               hb_storvc( pRetStr, -1, i );
               hb_xfree( pRetStr ); }
#else
...
#ifdef UNICODE  //20210926 kenny ooi
          {pRetStr = HMG_WCHAR_TO_CHAR( &buffer[0] );
           hb_retc( pRetStr );
           hb_xfree( pRetStr ); }
#else
           HMG_retc( &buffer[0] );
...
#ifdef UNICODE  //20210926 kenny ooi
   LPWSTR lpsDefFname, lpsIniDir, lpsTitle;
   LPSTR  pRetStr;
#endif
...
//**********************************************************************//   
...
#ifdef UNICODE  //20210926 kenny ooi
   lpsDefFname = HMG_parc(5);
   lstrcpy( buffer, lpsDefFname );
   if( !HB_ISNIL(3) ) {
   hb_xfree( lpsDefFname ); }
#else
 ...
 ofn.nMaxFile = sizeof(buffer) / sizeof(TCHAR);
#ifdef UNICODE  //20210926 kenny ooi
 lpsIniDir = HMG_parc(3);
 lpsTitle = HMG_parc(2);
 ofn.lpstrInitialDir = lpsIniDir ;
 ofn.lpstrTitle = lpsTitle ;
#else
 ofn.lpstrInitialDir = (LPCTSTR) HMG_parc(3);
 ofn.lpstrTitle = (LPCTSTR) HMG_parc(2);
#endif
...
#ifdef UNICODE  //20210926 kenny ooi
   lpsIniDir = HMG_parc (5);
   BrowseForFolderText.cInitPath = lpsIniDir ; //(TCHAR *) HMG_parc (5);
#else
...
#ifdef UNICODE  //20210926 kenny ooi
      pRetStr = HMG_WCHAR_TO_CHAR( cBuffer );
      hb_retc( pRetStr );
      hb_xfree( pRetStr );
#else
      HMG_retc (cBuffer);
